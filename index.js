...
function sortCarInventory(array) {
  ...
  const sorted_array = array.sort((x,y) => {
...
*/
function getOlderCars(array, max_year) {
  /* code here */
...
function getGermanCars(array) {
  ...
  for(let i = 0; i < array.length; i++)
  {
    let current_make = array[i].car_make;
    if(current_make === 'Audi' || current_make === 'Mercedes-Benz' || current_make === 'Volkswagen' || current_make === 'BMW')
